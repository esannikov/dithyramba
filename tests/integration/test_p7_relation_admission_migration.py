...
def test_schema_v8_fresh_apply_and_migration_copies_are_identical() -> None:
    ...
    try:
        ...
        assert [migration.version for migration in runner.apply_all()] == list(range(1, 11))
...
def test_v7_to_v8_zero_relation_rows_is_backup_first(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert [migration.version for migration in applied] == [8, 9, 10]
        assert backed_up == [8, 9, 10]
        assert runner.current_version() == 10
        assert (
